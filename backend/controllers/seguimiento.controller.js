...
async function aceptarTema(recepcion_id, usuario_id) {
  const [[row]] = await db.query(
    `
    SELECT estado FROM recepciones
    WHERE id=?
  `,
    [recepcion_id],
  );
...
`,
  ...
  const templatePath = path.join(__dirname, '../templates/invitacion.html');
  const template = fs.readFileSync(templatePath, 'utf8');

  const formularioPath = path.join(__dirname, '../templates/formulario.html');
  const formularioTemplate = fs.readFileSync(formularioPath, 'utf8');
  ...
  const urlQR = 'https://posgrado.usfx.bo/modalidad-investigacion-diplomado/';
